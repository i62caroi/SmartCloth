/*
    ...
    - BARCODE(EAN, nombre_comercial, peso_envase, id_tipo) 
        TIPO_ALIMENTO.id_tipo --> BARCODE.id_tipo
    
    - GRUPO_ALIMENTO(grupo, nombre, ejemplos, id_tipo)
...
CREATE TABLE Comida (
    ...
    `fecha` timestamp NOT NULL -- fecha y hora de la comida en UNIX
);
-- --------------------------------------------------------------------
...
CREATE TABLE GrupoAlimento (
    ...
    nombre VARCHAR(255),    -- nombre del grupo de alimentos
...
CREATE TABLE Barcode (
    ...
    nombre_comercial VARCHAR(100),  -- nombre comercial del producto
    peso_envase FLOAT,       -- peso base del envase
    id_tipo INT NOT NULL,           -- tipo de alimento al que hace referencia
