...
#define RA8876_TEXT_SPECIFIED_BACKGROUND   0
#define RA8876_TEXT_ORIGINAL_BACKGROUND    1
#define RA8876_TEXT_NO_ROTATION  0
#define RA8876_TEXT_ROTATION     1  
enum TextWidthScale{
  RA8876_TEXT_W_SCALE_X1 = 0x00,
  RA8876_TEXT_W_SCALE_X2 = 0x01,
  RA8876_TEXT_W_SCALE_X3 = 0x02,
  RA8876_TEXT_W_SCALE_X4 = 0x03
};
enum TextHeightScale{
  RA8876_TEXT_H_SCALE_X1 = 0x00,
  RA8876_TEXT_H_SCALE_X2 = 0x01,
  RA8876_TEXT_H_SCALE_X3 = 0x02,
  RA8876_TEXT_H_SCALE_X4 = 0x03
};
...
private:
  ...
  //uint16_t m_textForegroundColor;
  //uint16_t m_textBackgroundColor;
  //int      m_textScaleX;
  //int      m_textScaleY;
  TextWidthScale m_textScaleX;
  TextHeightScale m_textScaleY;
  ...
  void checkWriteFifoEmpty(void);                 // ICR => checkWriteFifoEmpty() en RA8876_Lite
 ...
 //----
  inline void waitWriteFifo(void) { while (readStatus() & 0x80); };       //checkWriteFifoNotFull() en RA8876_Lite, pero bloqueante
  inline void waitTaskBusy(void) { while (readStatus() & 0x08); };        //check2dBusy() en RA8876_Lite, pero bloqueante

  bool calcPllParams(uint32_t targetFreq, int kMax, PllParams *pll);      // Incluido en ra8876PllInitial() en RA8876_Lite
  bool calcClocks(void);                                                  // Incluido en ra8876PllInitial() en RA8876_Lite
  void dumpClocks(void);                                                  // Incluido en ra8876PllInitial() en RA8876_Lite

  bool initPLL(void);                                                     // ra8876PllInitial() en RA8876_Lite
  bool initMemory(SdramInfo *info);                                       // ra8876SdramInitial() en RA8876_Lite
  bool initDisplay(void);                                                 // ra8876Initialize() en RA8876_Lite
  /* ------------------------------------------------------------ */
  ...
  /* -------- TEXT/GRAPHICS MODE --------------------------------- */
  void setTextMode(void);                 // textMode() en RA8876_Lite
  void setGraphicsMode(void);
  /* ------------------------------------------------------------ */

  /* -------- BASIC SHAPES -------------------------------------- */
  // Low-level shapes
  void drawTwoPointShape(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color, uint8_t reg, uint8_t cmd);                  // drawLine, drawRect, fillRect
  void drawThreePointShape(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t x3, uint16_t y3, uint16_t color, uint8_t cmd);   // drawTriangle, fillTriangle
  void drawEllipseShape(uint16_t x, uint16_t y, uint16_t xrad, uint16_t yrad, uint16_t color, uint8_t cmd);                              // drawCircle, fillCircle
  void drawRoundRectShape(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t radius, uint16_t color, uint8_t cmd);             //drawRoundRect y fillRoundRect
  /* ------------------------------------------------------------ */
...
public:
  ...
  // Text cursor
  void setCursor(int x, int y);                 // setTextCursor() en RA8876_Lite
  int getCursorX(void);
  int getCursorY(void);
  ...
  void setTextParameter1(uint8_t source_select,FontSize size,CharacterSelection iso_set);//cch
      // source_select = 0 (selectInternalFont()), source_select = 1 (selectExternalFont())
  void setTextParameter2(uint8_t align, uint8_t chroma_key, TextWidthScale width_scale, TextHeightScale height_scale);//cdh
 
  void genitopCharacterRomParameter(uint8_t scs_select, uint8_t clk_div, ExternalFontRom rom, FontEncoding enc, ExternalFontFamily font);//b7h,bbh,ceh,cfh
  ...
  void setTextColor(uint16_t f_color, uint16_t b_color){ setTextForegroundColor(f_color); setTextBackgroundColor(b_color); };  // Texto y resaltado
  void ignoreTextBackground();
  
  //void setTextScale(int scale) { setTextScale(scale, scale); };
  void setTextScale(TextWidthScale width_scale, TextHeightScale height_scale);
  ...
  void putString(uint16_t x0,uint16_t y0, char *str); // ESTE ES CLAVE
  ...
  /* -------- GRAPHIC -------------------------------------------- */
  // Graphic cursor
  void setPixelCursor(uint16_t x,uint16_t y);               // ICR => setPixelCursor() en RA8876_Lite
  
  // Drawing
  void drawPixel(uint16_t x, uint16_t y, uint16_t color);
        // putPixel_16bpp() en RA8876_Lite
  void drawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color) { drawTwoPointShape(x1, y1, x2, y2, color, RA8876_REG_DCR0, RA8876_DRAW_LINE); }; //0x80
        // drawLine() en RA8876_Lite
  void drawRect(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color) { drawTwoPointShape(x1, y1, x2, y2, color, RA8876_REG_DCR1, RA8876_DRAW_SQUARE); }; //0xA0
        // drawSquare() en RA8876_Lite
  void fillRect(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color) { drawTwoPointShape(x1, y1, x2, y2, color, RA8876_REG_DCR1, RA8876_DRAW_SQUARE_FILL); }; //0xE0
        // drawSquareFill() en RA8876_Lite
  void drawRoundRect(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t radius, uint16_t color) { drawRoundRectShape(x1, y1, x2, y2, radius, color, RA8876_DRAW_ROUND_RECT); }; //0xB0
        // drawCircleSquare() en RA8876_Lite
  void fillRoundRect(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t radius, uint16_t color) { drawRoundRectShape(x1, y1, x2, y2, radius, color, RA8876_DRAW_ROUND_RECT_FILL); }; //0xF0
        // drawCircleSquareFill() en RA8876_Lite
  void drawTriangle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t x3, uint16_t y3, uint16_t color) { drawThreePointShape(x1, y1, x2, y2, x3, y3, color, 0xA2); }; //RA8876_DRAW_TRIANGLE => 0x82
        // drawTriangle() en RA8876_Lite
  void fillTriangle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t x3, uint16_t y3, uint16_t color) { drawThreePointShape(x1, y1, x2, y2, x3, y3, color, 0xE2); }; //RA8876_DRAW_TRIANGLE_FILL => 0xA2
        // drawTriangleFill() en RA8876_Lite
  void drawCircle(uint16_t x, uint16_t y, uint16_t radius, uint16_t color) { drawEllipseShape(x, y, radius, radius, color, RA8876_DRAW_CIRCLE); }; //0x80
        // drawCircle() en RA8876_Lite  
  void fillCircle(uint16_t x, uint16_t y, uint16_t radius, uint16_t color) { drawEllipseShape(x, y, radius, radius, color, RA8876_DRAW_CIRCLE_FILL); }; //0xC0
        // drawCircleFill() en RA8876_Lite
  void drawEllipse(uint16_t x, uint16_t y, uint16_t radius, uint16_t color) { drawEllipseShape(x, y, radius, radius, color, RA8876_DRAW_ELLIPSE); }; //0x80
        // drawEllipse() en RA8876_Lite  
  void fillEllipse(uint16_t x, uint16_t y, uint16_t radius, uint16_t color) { drawEllipseShape(x, y, radius, radius, color, RA8876_DRAW_ELLIPSE_FILL); }; //0xC0
